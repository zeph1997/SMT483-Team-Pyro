...
def thermal_detect_fire():
    db.child("Locations").child(LOCATION).child("ThermalFire").set(1)
    db.child("Locations").child(LOCATION).child("Logs").update({datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S_%f"):{"Fire":"Fire Detected","Camera":"Thermal"}})
    return True

def thermal_detect_no_fire():
    db.child("Locations").child(LOCATION).child("ThermalFire").set(0)
    db.child("Locations").child(LOCATION).child("Logs").update({datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S_%f"):{"Fire":"Fire No Longer Detected","Camera":"Thermal"}})
    return True