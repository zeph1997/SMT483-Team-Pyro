...
im1 = ax.imshow(grid_z,vmin=10,vmax=100,cmap=plt.cm.RdBu_r) # plot image, with temperature bounds
...
while True:
    status,pixels,isFire = sensor.read_temp(pix_to_read) # read pixels with status
    ...
    if isFire:
        ...
        if isFireCounter > 12:
            isNotFireCounter = 0
            database.thermal_detect_fire()
    else:
        if isFireCounter > 0:
            isNotFireCounter += 1
            if isNotFireCounter > 12:
                isFireCounter = 0
                database.thermal_detect_no_fire()
    time.sleep(1./30) # 30 gives the best framerate for both the thermal and computer vision model
